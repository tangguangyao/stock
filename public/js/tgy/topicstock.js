var topicStock={};
//公共函数，提取评论信息
topicStock.textExtract=function(comment,name){
...
topicStock.commentTopic=function(myTopic,e,callback){
...
topicStock.comRe=function(comment){
...
topicStock.angular=function($http,$scope,uid,stockName,selfName){
	var $http=$http,
			$scope=$scope,
			uid=uid,
			stockName=stockName,
	...
	return {
		//初始化
		haveStockTopic:function(){
		  this.getStockTopic(uid,stockName,10,0);
		},
		//初始化
		init:function(){
			var _this=this;
		  ...
		  $scope.comStockTopic=function(myTopic,e){
		    topicStock.commentTopic(myTopic,e,function(){
		      //评论大于0 并且第一次打开
		  ...
		  $scope.submitComStockTopic=function(e,myTopic){
		    if(myTopic.topicCommentText==""){
		      ...
		      var commentObj=topicStock.textExtract(myTopic.topicCommentText,selfName);
		  ...
		  $scope.comStockRe=function(comment,myTopic){
		    ...
		    if(comment.commentReText==noText){
			    ...
			    var commentObj=topicStock.textExtract(comment.commentReText,selfName);
			    commentObj.pid=myTopic.uid;
